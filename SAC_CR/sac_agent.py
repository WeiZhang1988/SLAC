...
class SacAgent(object):
	def __init__(self, replay_buffer, actor_learning_rate=3e-4, \
			...
			batch_size=256, target_update_tau=0.005, target_update_period=1, \
	...
	def learn(self):
		...
		with tf.GradientTape(persistent=True) as tape:
			...
			td_target1 = self.reward_scale_factor * rewards + (1.0 - dones) * self.gamma * self.target_critic_network1(next_observations, next_new_policy_actions) - self.alpha * next_new_policy_log_probs
			td_target2 = self.reward_scale_factor * rewards + (1.0 - dones) * self.gamma * self.target_critic_network2(next_observations, next_new_policy_actions) - self.alpha * next_new_policy_log_probs
		...
		#------------------------------------------------------------
		with tf.GradientTape() as tape:
			alpha_loss = -self.alpha * (new_policy_log_probs + self.target_entropy) 
