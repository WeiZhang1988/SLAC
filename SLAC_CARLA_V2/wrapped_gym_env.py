...
class WrappedGymEnv(gym.Wrapper):
	def __init__(self, env, image_size=[64,64], steps_com=1):
		...
		self.action_low = self.action_space.low.astype("float32")
		self.action_high = self.action_space.high.astype("float32")
	...
	def reset(self):
		...
		obs = tf.image.resize(tf.convert_to_tensor(obs_raw['bev']), self.image_size).numpy().astype(np.float32) / self.observation_high
		return obs
	def step(self, action):
		...
		if done:
			...
			step_type = 0
		self.num_step += 1
		obs = tf.image.resize(tf.convert_to_tensor(obs_raw['bev']), \
